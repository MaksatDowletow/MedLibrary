...
> **Примечание.** Если MongoDB временно недоступна, API автоматически переключится на встроенное хранилище в памяти. Оно подходит для локального тестирования, но данные стираются при перезапуске сервера, поэтому для продакшена по-прежнему требуется полноценная база.
